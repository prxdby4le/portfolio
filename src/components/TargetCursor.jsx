...
/**
 * TargetCursor
 * Cursor animado que "encaixa" nos elementos com a classe .cursor-target
 * Props:
 *  - targetSelector: seletor CSS dos elementos alvo (default: '.cursor-target')
 *  - spinDuration: tempo em segundos para rotação contínua
 *  - hideDefaultCursor: se true, esconde o cursor padrão do sistema
 */
const TargetCursor = ({ 
      ...
      const leaveHandler = () => {
        ...
        resumeTimeout = setTimeout(() => {
          if (!activeTarget && cursorRef.current && spinTl.current) {
            ...
            gsap.to(cursorRef.current, { rotation: normalized + 360, duration: spinDuration * (1 - normalized / 360), ease: 'none', onComplete: () => spinTl.current?.restart() });
          }
          resumeTimeout = null;
        }, 50);
  ...
  return (
    <div ref={cursorRef} className={styles.targetCursorWrapper}>
      ...
      <div className={`${styles.targetCursorCorner} ${styles.cornerTl} target-cursor-corner`} />
      <div className={`${styles.targetCursorCorner} ${styles.cornerTr} target-cursor-corner`} />
      <div className={`${styles.targetCursorCorner} ${styles.cornerBr} target-cursor-corner`} />
      <div className={`${styles.targetCursorCorner} ${styles.cornerBl} target-cursor-corner`} />
    </div>
  );
};
